  ...
  const modal = (
    ...
    >
      <div
        data-id="modal"
        className={clsx(
          "bg-white mx-auto rounded-lg min-h-[320px] relative",
          "flex flex-col",
          {
...
UiModal.Header = function Header({ children, className }) {
  return (
    <div className={clsx(className, "px-6 pt-6 pb-4 text-2xl")}>{children}</div>
  );
};

UiModal.Body = function Body({ children, className }) {
  return <div className={clsx(className, "px-6")}>{children}</div>;
};

UiModal.Footer = function Footer({ children, className }) {
  return (
